...
export class DisplayHelperService {
  ...
  /**
   * Obtiene el SVG y texto para un tipo de notificación
   */
  ...
  /**
   * Obtiene el SVG y texto para un estado de invitación
   */
  ...
  /**
   * Obtiene el SVG y texto para una franja horaria
   */
  getFranjaHorariaDisplay(franja?: string): string {
    if (!franja) {
      return '<svg class="inline w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"/></svg> General';
    }
